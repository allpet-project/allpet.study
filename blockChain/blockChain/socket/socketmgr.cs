...
namespace blockChain.socket
{
    public class SocketManager
    {
        ...
        public delegate void OnReceiveMsgHandler(string ip,string msg);
        public event OnReceiveMsgHandler OnReceiveMsg;
        public delegate void OnDisConnectedHandler(string ip);
        ...
        private void ReceiveCallBack(IAsyncResult ar)
        {
            ...
            if (readCount > 0)
            {
                ...
                if (OnReceiveMsg != null) OnReceiveMsg(info.socket.RemoteEndPoint.ToString(),msgTip);
            }
        }

        public void SendMsg(string text, string endPoint)
        {
            if (_listSocketInfo.ContainsKey(endPoint) && _listSocketInfo[endPoint] != null)
            {
